...
class Splitter extends LinearLayout
{
    ...
    public Splitter(Context context, DataGrid.MemberCollection mc, int intIndex)
    {
        ...
        if (mc.COLUMN_STYLE.get(index).getWitdh() == 0)
        {
            isCollapsed = true;
        }
        ...
        tvTop1.setText("");
        tvTop1.setHeight(12);
        tvTop1.setBackgroundColor(DataGridStyle.SpliterCell.BackgroundColor);

        if (!isCollapsed)
        {
            tvTop1.setPadding(DataGridStyle.SpliterCell.LPadding, 0, DataGridStyle.SpliterCell.RPadding, 0);
        }
        else
        {
        ...
        if (!isCollapsed)
        {
            tvTop2.setPadding((DataGridStyle.HeaderSpliterCell.LPadding - DataGridStyle.SpliterCell.LPadding), 0, 0, 0);
        }
        else
        {
        ...
        {
            tvTop3.setPadding(0, 0, (DataGridStyle.HeaderSpliterCell.RPadding - DataGridStyle.SpliterCell.RPadding), 0);
        }
        else
        {
        ...
        tvBottom1.setBackgroundColor(DataGridStyle.SpliterCell.BackgroundColor);

        if (!isCollapsed)
        {
            tvBottom1.setPadding(DataGridStyle.SpliterCell.LPadding, 0, DataGridStyle.SpliterCell.RPadding, 0);
        }
        else
        {
        ...
        if (!isCollapsed)
        {
            tvBottom2.setPadding((DataGridStyle.HeaderSpliterCell.LPadding - DataGridStyle.SpliterCell.LPadding), 0, 0, 0);
        }
        else
        {
        ...
        if (!isCollapsed || index == 0) // in case of the first column in hidden column, panel on the right is still needed to fill up the space
        {
            tvBottom3.setPadding(0, 0, (DataGridStyle.HeaderSpliterCell.RPadding - DataGridStyle.SpliterCell.RPadding), 0);
        }
        else
        {
    ...
    public boolean onTouchEvent(MotionEvent e)
    {
        ...
        else if (e.getAction() == MotionEvent.ACTION_MOVE)
        {
            if (status == START_DRAGGING)
            {
                ...
                int headerSpilterWidth = (DataGridStyle.HeaderSpliterCell.LPadding + DataGridStyle.HeaderSpliterCell.RPadding);
                int dataSpliterWidth = (DataGridStyle.SpliterCell.LPadding + DataGridStyle.SpliterCell.RPadding);
